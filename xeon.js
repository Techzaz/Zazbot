...
async function startXeonBotInc() {
    const XeonBotInc = XeonBotIncConnect({
        ...
        browser: ['Subscribe Techzaz','Safari','1.0.0'],
        auth: state
    })
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   //dont forget to put my name(TechZaz) as credit
...
XeonBotInc.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
                	...
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('Asia/Karachi').format('HH:mm:ss')
	                const xeondate = moment.tz('Asia/Karachi').format('DD/MM/YYYY')
                	let xeonName = num
                    const xeonmembers = metadata.participants.length
                    let unicorndoc = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "923184365151-1604595598@g.us"}, "message": {orderMessage: {itemCount: 9999999,status: 200, thumbnail: xeonbuffer, surface: 200, message: `${metadata.subject}`, orderTitle: 'TechZaz', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
                    xeonbody = `┌─❖
│「 𝗚𝗼𝗼𝗱𝗯𝘆𝗲 👋 」
└┬❖ 「 @${xeonName.split("@")[0]}  」
   │✑  𝗟𝗲𝗳𝘁 
...
//send 5 button image by TechZaz
